...
from postvox.config.loader import (
    Cursor,
    deep_merge,
    env_overlay,
    expand_path,
...
from postvox.config.plugins import instantiate
from postvox.errors import ConfigError
# The canonical connector list lives with the pure text transform that uses it,
# so the config default and the code default cannot drift apart. postvox.text
# imports nothing from postvox.config, so this direction stays acyclic.
from postvox.text.shape import DEFAULT_TRAILING_CONNECTORS
from postvox.version import CONFIG_SCHEMA_VERSION, LEDGER_SCHEMA_VERSION, user_agent
...
_DEFAULT_STATE_DIR = os.path.join("~", ".local", "state", "postvox")
...
class LoggingConfig:
    ...
    def from_cursor(cls, cur: Cursor) -> "LoggingConfig":
        ...
        return cls(
            logger_name=cur.str("logger_name", "postvox"),
...
class CoverConfig:
    ...
    enabled: bool = True
    impl: str = "postvox.media.cover:BrightnessScanCoverPicker"
    ...
    def from_cursor(cls, cur: Cursor) -> "CoverConfig":
        ...
        return cls(
            ...
            impl=cur.str("impl", "postvox.media.cover:BrightnessScanCoverPicker"),
...
class IdentityConfig:
    ...
    impl: str = "postvox.identity.post_id:FilenamePostIdentity"
    options: Dict[str, Any] = field(default_factory=dict)
    ...
    def from_cursor(cls, cur: Cursor) -> "IdentityConfig":
        ...
        return cls(
            impl=cur.str("impl", "postvox.identity.post_id:FilenamePostIdentity"),
            options=options,
...
class LedgerConfig:
    ...
    impl: str = "postvox.state.ledger:JsonlLedger"
    options: Dict[str, Any] = field(default_factory=dict)
    ...
    def from_cursor(cls, cur: Cursor, state: StateConfig) -> "LedgerConfig":
        ...
        return cls(
            impl=cur.str("impl", "postvox.state.ledger:JsonlLedger"),
            options=options,
...
class LinksConfig:
    ...
    tagger: PluginSpec = field(
        default_factory=lambda: PluginSpec("postvox.text.links:NoopTagger", {})
    )
    ...
    def from_cursor(cls, cur: Cursor) -> "LinksConfig":
        ...
        tagger = (
            ...
            if tagger_cur.data
            else PluginSpec("postvox.text.links:NoopTagger", {})
        )
    ...
    def from_cursor(cls, cur: Cursor, facebook: FacebookConfig) -> "InstagramConfig":
        ...
        if enabled and staging is None:
            raise ConfigError(
                ...
                "postvox.platforms.meta.staging).",
...
class XGovernorConfig:
    ...
    state: PluginSpec = field(
        default_factory=lambda: PluginSpec("postvox.state.counters:JsonCounterStore", {})
    )
    ...
    def from_cursor(cls, cur: Cursor, state_cfg: StateConfig) -> "XGovernorConfig":
        ...
        if state_cur.data:
            ...
            spec = PluginSpec("postvox.state.counters:JsonCounterStore", {})
...
class XPricingConfig:
    """Used ONLY by ``postvox estimate`` / dry-run output.
...
class CliConfig:
    ...
    def from_cursor(cls, cur: Cursor) -> "CliConfig":
        ...
        return cls(
            program_name=cur.str("program_name", "postvox"),
...
class Config:
    ...
    def load(
        ...
        *,
        ...
        disk — ``postvox`` is usable with environment variables alone.

        Precedence, lowest to highest: file, ``POSTVOX_*`` environment overlay,
