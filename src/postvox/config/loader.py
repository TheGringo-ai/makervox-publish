...
Search order (first file wins), overridable with ``POSTVOX_CONFIG=/path/to.toml``:
...
CONFIG_ENV_VAR = "POSTVOX_CONFIG"
ENV_OVERLAY_PREFIX = "POSTVOX_"
_BASENAMES = ("postvox.toml", "postvox.yaml", "postvox.yml", "postvox.json")
...
def search_paths(cwd: Optional[str] = None) -> List[str]:
    """The ordered candidate list, honouring ``POSTVOX_CONFIG``."""
    ...
    for base in _BASENAMES:
        out.append(os.path.join(_xdg_config_home(), "postvox", base))
    for base in _BASENAMES:
        out.append(os.path.join("/etc", "postvox", base))
    return out
    ...
    try:
        ...
        try:
            ...
            raise ConfigError(
                ...
                "pip install 'postvox[toml]' (or use a .json config)",
                source=path,
            ) from exc
    try:
...
def _load_yaml(raw: bytes, path: str) -> Dict[str, Any]:
    try:
        ...
        raise ConfigError(
            "reading YAML needs the yaml extra: pip install 'postvox[yaml]' "
...
def env_overlay(environ: Optional[Mapping[str, str]] = None) -> Dict[str, Any]:
    """Build an overlay mapping from ``POSTVOX_``-prefixed environment variables.
    ...
    arrive as the right type. ``POSTVOX_CONFIG`` is excluded: it selects the
