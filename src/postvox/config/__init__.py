"""postvox.config — one config surface, one place defaults live.

    from postvox.config import Config

    cfg = Config.load()                       # searches for postvox.toml
    cfg = Config.load("/etc/postvox/postvox.toml")
...
from postvox.config.plugins import instantiate, resolve
