...
* Every invocation has a timeout. An ffmpeg that hangs on a malformed input
  ...
  logs.
* Failures raise :class:`~postvox.errors.FfmpegFailed` carrying the tail of
...
from postvox.errors import FfmpegFailed, FfmpegNotFound
...
class FfmpegTools:
    ...
    def from_config(cls, media_cfg) -> "FfmpegTools":
        """Build from a :class:`postvox.config.MediaConfig`."""
        return cls.discover(
