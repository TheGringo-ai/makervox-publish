...
2. Resolved secrets are registered with :func:`register_secret` and scrubbed
   from every record emitted under the ``postvox`` logger tree, so an
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    """Return a child of the ``postvox`` logger.
...
def register_secret(value: Optional[str]) -> None:
    """Mark a resolved secret so it is scrubbed from every postvox log record."""
...
class SecretRedactionFilter(logging.Filter):
    ...
    Attached to the ``postvox`` logger by :func:`configure`. Filters on a
...
def configure(
    ...
    *,
    ...
    if handler is not None:
        if not any(getattr(h, "_postvox_handler", False) for h in log.handlers):
            handler._postvox_handler = True  # type: ignore[attr-defined]
...
def configure_from_config(logging_cfg, *, redact: bool = True,
    ...
    """Apply a :class:`postvox.config.LoggingConfig`."""
    return configure(
