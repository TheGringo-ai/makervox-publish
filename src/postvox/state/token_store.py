...
from postvox.errors import ConfigError, PostvoxError
from postvox.logging import get_logger, swallowed
from postvox.state.atomic import read_json, write_json
from postvox.state.locks import file_lock
from postvox.state.paths import expand
...
class LockSettings:
    ...
    def from_config(cls, state_cfg) -> "LockSettings":
        ...
        state_dir = expand(getattr(state_cfg, "dir", "~/.local/state/postvox"))
        return cls(
...
class TokenStore(Protocol):
    ...
    def describe(self) -> str:
        """One line for ``postvox doctor``. Never a token value."""
        ...
...
class FileTokenStore(_CachedStore):
    ...
    def save(self, data: Dict[str, Any]) -> None:
        if self.atomic_write:
            ...
            from postvox.state.paths import chmod_quietly, ensure_parent
...
class SecretManagerTokenStore(_CachedStore):
    ...
    ``use_cli = false``
        the ``google-cloud-secret-manager`` library (``pip install postvox[gcp]``)
    ``use_cli = true``
    ...
    def _build_client(self):
        try:
            ...
            raise ConfigError(
                ...
                "library: pip install 'postvox[gcp]'. Or set use_cli = true to "
...
def build_token_store(spec, state_cfg, *, cache_ttl_s: Optional[float] = None):
    ...
    ``spec`` is a :class:`postvox.config.PluginSpec` from ``[token_stores.<name>]``.
    ...
    try:
        return spec.build(group="postvox.token_stores", **extras)
    except PostvoxError:
        raise
