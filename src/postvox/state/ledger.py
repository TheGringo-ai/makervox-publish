...
``postvox.identity.post_id`` for why a filename is not an identity.
...
from postvox.version import LEDGER_SCHEMA_VERSION
...
class JsonlLedger(Ledger):
    ...
    def from_config(cls, ledger_cfg, state_cfg=None) -> "JsonlLedger":
        """Build from a :class:`postvox.config.LedgerConfig` (+ ``[state]``)."""
