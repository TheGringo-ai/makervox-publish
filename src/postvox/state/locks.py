...
-------------------------------------
A module-level ``import fcntl`` makes ``import postvox`` fail outright on
...
from postvox.errors import ConfigError, LockUnavailable
from postvox.logging import get_logger
from postvox.state.paths import ensure_dir, under
...
_DEFAULT_LOCK_DIR = os.path.join("~", ".local", "state", "postvox", "locks")
...
class LockPolicy:
    ...
    def from_config(cls, state_cfg: Any) -> "LockPolicy":
        """Build from a :class:`postvox.config.StateConfig` (or anything alike)."""
