...
:mod:`postvox.platforms.meta.staging`. A staged object that fails to delete stays
...
from postvox.credentials import CredentialSpec, Credentials
from postvox.errors import ConfigError, IdentityClash, PublishError
from postvox.http.client import HttpClient, TransportError
...
from postvox.platforms.meta.staging import content_type_for, staged
from postvox.platforms.meta.tokens import MetaTokenStore, PageCredentials, StoredPageResolver
...
_SETUP = (
    ...
    "Page: store an Instagram-login token with `postvox auth instagram <account>`."
)
...
class InstagramPublisher:
    ...
    def stager(self) -> Any:
        if self._stager is None:
            ...
            self._stager = self.cfg.staging.build(group="postvox.media_stagers")
        return self._stager
...
def publisher(config: Any, **kwargs: Any) -> InstagramPublisher:
    """Factory used by :func:`postvox.platforms.publisher_for`."""
