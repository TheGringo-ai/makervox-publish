...
from postvox.text.shape import shape_trace
...
def shape(caption: str, shape_config=None) -> str:
    ...
    """
    from postvox.config.schema import XShapeConfig
    from postvox.text.shape import shape_with_config
...
class XClient:
    ...
    def from_config(cls, cfg, *, credentials: Optional[Credentials] = None,
        ...
        """Build from a whole :class:`postvox.config.Config`."""
        x_config = cfg.platforms.x
        creds = credentials or Credentials.from_config(cfg.credentials)
        tagger = None
        try:
            tagger = cfg.links.tagger.build(group="postvox.link_taggers")
    ...
    def _auth(self) -> Tuple[Any, Optional[str]]:
        ...
        try:
            ...
            return None, (
                ...
                "1.0a signing. Install it with: pip install \"postvox[x]\""
            )
        return OAuth1(
    ...
    def estimate(self, *, posts: int = 1, link_reply: Optional[bool] = None) -> str:
        """Cost of ``posts`` posts under the current settings, for ``postvox estimate``."""
        pricing = self.config.pricing
    ...
    def status(self) -> Dict[str, Any]:
        """A credential-free summary for ``postvox doctor``."""
        ready, missing = self.ready()
        return {
            "platform": self.platform,
