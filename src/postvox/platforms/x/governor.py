...
The counter itself is a :class:`~postvox.state.counters.CounterStore`; see that
...
from postvox.state.counters import CounterStore, JsonCounterStore
from postvox.text.shape import fingerprint
...
class Governor:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.store = store if store is not None else JsonCounterStore("postvox_x_governor.json")
    ...
    def from_config(cls, x_config, *, store: Optional[CounterStore] = None,
        ...
        """Build from a :class:`postvox.config.schema.XConfig`.
