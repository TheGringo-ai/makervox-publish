"""KeyringProvider — the OS keychain. Optional extra: pip install postvox[keyring].
...
class KeyringProvider:
    def __init__(self, service: str = "postvox") -> None:
        try:
            import keyring as _keyring  # noqa: WPS433 - optional extra
        except ImportError as exc:
            raise ProviderUnavailable(
                "the keyring extra is not installed: pip install 'postvox[keyring]'"
            ) from exc
        try:
