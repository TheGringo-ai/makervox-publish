...
from makervox_publish.errors import FfmpegFailed, MediaError
...
class CoverPick:
    ...
    so ``makervox_publish doctor`` and the tests can assert on the REASON, not only on
    the final integer.
    """
...
class BrightnessScanCoverPicker:
    ...
    def from_config(cls, cover_cfg, tools: Optional[FfmpegTools] = None
        ...
        """Build from a :class:`makervox_publish.config.CoverConfig`."""
        return cls(
...
def cover_offset_ms(
    ...
    *,
    ...
    ``config`` is a :class:`makervox_publish.config.CoverConfig`; ``picker`` overrides it
