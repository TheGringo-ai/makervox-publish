"""makervox_publish.credentials — pluggable secret resolution.
...
from makervox_publish.credentials.base import (
    ChainProvider,
    CredentialError,
    CredentialNotFound,
    CredentialProvider,
...
from makervox_publish.credentials.env import EnvProvider
from makervox_publish.credentials.static import StaticProvider
from makervox_publish.logging import get_logger, register_secret
...
class Credentials:
    ...
    def from_config(cls, cfg: Optional[object]) -> "Credentials":
        ...
        against the ``makervox_publish.credential_providers`` entry-point group, so third
        ...
        """
        from makervox_publish.config.plugins import instantiate  # layered below us
            ...
            try:
                providers.append(
                    instantiate(impl, options, key=key,
                                group="makervox_publish.credential_providers")
                )
    ...
    def status(self, spec: Mapping[str, Sequence[str]]) -> Dict[str, dict]:
        """``{platform: {"ready", "missing", "keys"}}`` for ``makervox_publish doctor``.
