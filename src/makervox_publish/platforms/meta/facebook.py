...
from makervox_publish.media.shortcut import short_cut_for_publish
from makervox_publish.platforms import PublishResult
from makervox_publish.platforms.meta import compat
from makervox_publish.platforms.meta.graph import GraphEndpoints, GraphSession
...
from makervox_publish.state.ledger import JsonlLedger, LedgerRow
from makervox_publish.state.locks import file_lock
from makervox_publish.text.links import NoopTagger, readable_text, strip_from_config
...
class FacebookPublisher:
    ...
    def from_config(cls, config, *, credentials: Optional[Credentials] = None,
        ...
        """Wire everything from a :class:`makervox_publish.config.Config`."""
        ...
        if spec is not None:
            try:
                tagger = spec.build(group="makervox_publish.link_taggers")
            except Exception as exc:
    ...
    def page(self, account: str) -> PageCredentials:
        ...
        if page is None:
            raise PublishError(
                ...
                "Page (`makervox_publish auth facebook {0}`) or set "
    ...
    def link(self, account: str, page_id: str) -> str:
        ...
        with self.tokens.transaction() as txn:
            ...
            if not record:
                raise PublishError(
                    "Page {0} is not connected yet — run `makervox_publish auth facebook` "
    ...
    def set_reel_cover(self, video_id: str, video_path: str, token: str) -> bool:
        ...
        The frame choice itself is :func:`makervox_publish.media.cover.cover_offset_ms`,
...
def publisher(config, **kwargs) -> FacebookPublisher:
    """Entry point used by :func:`makervox_publish.platforms.publisher_for`."""
